...
"""
class maphandler:
    def __init__(self, mapfilename, playersprite, playercoords,
                 framedict, screensize, tilesize=32):
        ...
        self.load(mapfilename, (48,31)) # this creates the map arrays
        ...
        # self.eventlist    = 3d array for events. see load function for more info
    ...
    def load(self, mapname, targetcoords):
        ...
        for child in root:
            if child.tag == "layer":
                if not child.attrib["name"] == "collision" or child.attrib["name"] == "event":
        ...
        for child in root:
            if child.tag == "layer":
                if child.attrib["name"] == "event":
                    layer = child.findtext("data").replace("\n", "").split(",")
                    for tile in layer:
                        container.append([int(tile)])#making each "tile" a single item list
                        # these being lists should create a third dimension to the lists which
                        # can be looked at
                    eventlist = container
                    container = []
                    eventlist = self.decipher(eventlist) # turns the list into an array.
                    # now that the array is 3d we will append to the z dimension the data
                    # this will add what should happen to the end of the list
                    # later there will need to be different handlers based on the property names
                    # this data is in the following format:
                    # [triggertype, coords, eventtype, mapname, (target coords)]
                    for event in child.find("properties"):
                        val = event.attrib["value"].split(",")
                        coords = (int(val[0]),int(val[1])) #coords
                        eventlist[coords[1]][coords[0]].append(int(val[2])) #eventtype
                        eventlist[coords[1]][coords[0]].append(val[3])#mapname - string
                        eventlist[coords[1]][coords[0]].append((int(val[4]), int(val[5]))) #(target coords)
                    self.eventlist = eventlist
                            
        """
        ##--------------------------------------------------------##
        ## big bit here about calculating where to place the player
        ##--------------------------------------------------------##
        """
        # determine where to draw the map
        # first validate that the coords are in-bounds
        if targetcoords[0] > width or targetcoords[0] < 0:
            print("Target X was out of bounds during load")
        if targetcoords[1] > height or targetcoords[1] < 0:
            print("Target Y was out of bounds during load")
        
        # calculate pixel coords from tile position
        targetpixelX    = targetcoords[0] * self.tilesize
        targetpixelY    = targetcoords[1] * self.tilesize
        mapwidthpix     = width*self.tilesize
        mapheightpix    = height*self.tilesize

        # begin checking where the map offset should be placed
        # this will have an X check segment and then a y check segment
        # check X
        if mapwidthpix - targetpixelX < (self.screenwidth//2):
            # right map edge is not far enough over if player is centered
            self.mapoffset[0] = self.screenwidth - mapwidthpix
            self.playercoords[0] = self.screenwidth - (mapwidthpix-targetpixelX)
        elif targetpixelX < (self.screenwidth//2):
            # player is too close to the left edge to draw centered
            self.mapoffset[0] = 0
            self.playercoords[0] = targetpixelX
        else: # player can be centered - determine map x offset
            self.mapoffset[0] = -(targetpixelX - (self.screenwidth//2))
            self.playercoords[0] = (self.screenwidth//2)
        # check Y
        if mapheightpix - targetpixelY < (self.screenheight//2):
            # bottom of map edge will not fit if player is centered
            self.mapoffset[1] = self.screenheight - mapheightpix
            self.playercoords[1] = self.screenheight - (mapheightpix-targetpixelY)        
        elif  targetpixelY < (self.screenheight//2):
            # player is too close to the top of the map to draw centered
            self.mapoffset[1] = 0
            self.playercoords[1] = targetpixelY
        else: # player can be centered Y determine map y offset
            self.mapoffset[1] = -(targetpixelY - (self.screenheight//2))
            self.playercoords[1] = (self.screenheight//2)
    ...
    ##--------------------------------------------------------##
    """
    # this should be called if we notice the player is triggering an event
    def handleEvent(self, triggertype):
        # set local variables the function will need to handle this
        playerindexY = int(self.playercoords[1]/32) - int(self.mapoffset[1]/32)
        playerindexX = int(self.playercoords[0]/32) - int(self.mapoffset[0]/32)
        
        # at this point we know that something caused the player box to enter an event tile
        # We need to determine what
        if triggertype == "onstep":
            # we know where on the map the player is so we will look at that part of the event array
            Y = playerindexY
            X = playerindexX
            event = self.eventlist[Y][X] # this allows us to index this easier
            print(event)
            print(len(event))
            print(X,Y)
            if event[1] == 1: # this is a load event
                self.load(event[2], event[3])
                self.render()
                
        else: # this was most likely an "activation" with space bar
            """placeholder - this will be for activation type events"""
    """
    ##--------------------------------------------------------##
    ## get events - works like get walls
    ##--------------------------------------------------------##
    """    
    # this function will check and trigger when the player interacts
    # with an event tile. It will need to determine the trigger
    # source and act accordingly
    # This will be similar to the walls check, however it will look for
    # more than just 1, and will generate a list of "event tiles"
    def getevents(self):
        playerindexY = int(self.playercoords[1]/32) - int(self.mapoffset[1]/32)
        playerindexX = int(self.playercoords[0]/32) - int(self.mapoffset[0]/32)
        output = []
    ...
    def handleInput(self, keyspressed):
        ...
        for key in keyspressed:
            if key == "K_UP":
                ...
                if self.playerbox.collidelist(events) != -1: 
                    self.playercoords[1] -= 32 # player coords need to be at this location when the event handling occurs
                    self.handleEvent("onstep")
                    break
                if self.playerbox.collidelist(walls) == -1: # if there is no collision
                    if self.mapoffset[1] >= 0: # if the map is at the top of the screen
                        if self.playercoords[1] >= 32: # if the player is not at the top of the screen
                            self.playercoords[1] -= 32 # move the player up
                            output = "step"
            ...
            if key == "K_DOWN":
                ...
                if self.playerbox.collidelist(events) != -1:
                    self.playercoords[1] += 32 # player coords need to be at this location when the load occurs
                    self.handleEvent("onstep")
                    break
            ...
            if key == "K_LEFT":
                ...
                if self.playerbox.collidelist(events) != -1:
                    self.playercoords[0] -= 32 # player coords need to be at this location when the load occurs
                    self.handleEvent("onstep")
                    break
                if self.playerbox.collidelist(walls) == -1:
                    if self.mapoffset[0] >= 0:
                        if self.playercoords[0] >= 32:
                            self.playercoords[0] -= 32
                            output = "step"
            ...
            if key == "K_SPACE": # this will be an action key later, for now it is map debug
                print("-----------------------------")
                print("debug")
                print("-----------------------------")
                print("collision: ", self.playerbox.topleft)
                print("player coords: ", self.playercoords)
                print("screen tile location: ", int(self.playercoords[0]/32), int(self.playercoords[1]/32))
                print("Map offset: ", self.mapoffset)
            return output
